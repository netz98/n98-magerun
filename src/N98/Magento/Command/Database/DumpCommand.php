...
class DumpCommand extends AbstractDatabaseCommand
{
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption('data-to-csv', 'csv', InputOption::VALUE_NONE, 'Dump table data to csv files.')
    ...
    protected function getDataToCsvHelp()
    {
        ...
        $messages[] = ' Speedups export by 10-20%';
    ...
    protected function getExecToDumpDbDataAsCsv($folderName, array $stripTables)
    {
        ...
        if (!$this->mysqlUserHasFilePrivilege()) {
...
SQL;
     ...
     * @return bool|string
     */
    private function detectMysqlTmpDir()
    {
    ...
    /**
     * Check whether current mysql user has FILE privilege
     *
     * @return bool
     */
    private function mysqlUserHasFilePrivilege()
    {
        ...
        if (isset($commandOutput[1])) {
            ...
            foreach ($commandOutput as $line) {
                if (preg_match('/^GRANT(.*)FILE/', $line) || preg_match('/^GRANT(.*)ALL/', $line)) {
                    return true;
                }
            }
        }
