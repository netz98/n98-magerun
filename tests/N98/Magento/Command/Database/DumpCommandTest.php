...
final class DumpCommandTest extends TestCase
{
    ...
    public function provideFilenamePatternsAndOptions(): \Iterator
    {
        ...
        yield ['/^.*\d{4}-\d{2}-\d{2}_\d{6}\.sql$/', []];
        # testAddTimePrefixAutogenerated
        yield ['/^\d{4}-\d{2}-\d{2}_\d{6}.*\.sql$/', ['--add-time' => 'prefix']];
        # testAddTimeFilenameSpecified
        yield ['/^.*\d{4}-\d{2}-\d{2}_\d{6}\.sql.gz$/', ['--compression' => 'gzip']];
        # testAddTimeFilenameSpecified
        yield ['/^foo_\d{4}-\d{2}-\d{2}_\d{6}\.sql$/', ['filename' => 'foo.sql']];
        # testAddTimePrefixFilenameSpecified
        yield ['/^\d{4}-\d{2}-\d{2}_\d{6}_foo\.sql$/', ['filename' => 'foo.sql', '--add-time' => 'prefix']];
        # testAddTimeOffFilenameSpecified
        yield ['/^foo.sql$/', ['filename' => 'foo.sql', '--add-time' => 'no']];
